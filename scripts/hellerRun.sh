...
if [[ $inFile == *"store"* ]]
then
    cmsRun bmaker/python/bmaker_heller_cfg.py inputFiles=$inFile nEvents=$nEvents json=$json $outName 2>&1 | tee logout.log
else 
    cmsRun bmaker/python/bmaker_heller_cfg.py inputFiles=file:$inFile nEvents=$nEvents json=$json $outName 2>&1 | tee logout.log
fi

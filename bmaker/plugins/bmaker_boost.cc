//// BMAKER_BOOST: Creates baby tree with boost branches
...
// User include files
#include "babymaker/bmaker/interface/bmaker_boost.hh"
#include "babymaker/bmaker/interface/baby_boost.hh"
...
void bmaker_boost::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  nevents++;
  ...
  if(isData){
    ...
    // We are applying the golden JSON with lumisToProcess in bmaker_boost_cfg.py
  ...
  for (const pat::PackedCandidate &pfc : *pfcands) {
    baby.pfcands_id().push_back(pfc.pdgId());
    baby.pfcands_charge().push_back(pfc.charge()!=0);
    baby.pfcands_pt().push_back(pfc.pt());
    baby.pfcands_m().push_back(pfc.mass());
    baby.pfcands_eta().push_back(pfc.eta());
    baby.pfcands_phi().push_back(pfc.phi());
    baby.pfcands_frompv().push_back(pfc.fromPV());
  } 
  ...
  if (!isData) {
    ...
    edm::Handle<edm::View<pat::PackedGenParticle> > packedParticles;
    iEvent.getByLabel("packedGenParticles", packedParticles);

    writeMC(genParticles, packedParticles, all_mus, all_els, photons);
  }
...
void bmaker_boost::writeMET(edm::Handle<pat::METCollection> mets, edm::Handle<pat::METCollection> mets_nohf){
...
// Requires having called jetTool->getJetCorrections(alljets, rhoEvent_) beforehand
void bmaker_boost::writeJets(edm::Handle<pat::JetCollection> alljets, 
                             edm::Handle<edm::View <reco::GenJet> > genjets,
...
void bmaker_boost::writeFatJets(vector<LVector> &jets){
...
vCands bmaker_boost::writeMuons(edm::Handle<pat::MuonCollection> muons, 
                                edm::Handle<pat::PackedCandidateCollection> pfcands, 
                                edm::Handle<reco::VertexCollection> vtx,
...
vCands bmaker_boost::writeElectrons(edm::Handle<pat::ElectronCollection> electrons, 
                                    edm::Handle<pat::PackedCandidateCollection> pfcands, 
                                    edm::Handle<reco::VertexCollection> vtx,
...
void bmaker_boost::writeLeptons(vCands &leptons){ 
...
void bmaker_boost::writeDiLep(vCands &sig_mus, vCands &sig_els, vCands &veto_mus, vCands &veto_els){
...
void bmaker_boost::setDiLepMass(vCands leptons, baby_float ll_m, baby_float ll_pt1, baby_float ll_pt2, 
...
void bmaker_boost::setElMuMass(vCands leptons1, vCands leptons2, baby_float ll_m, baby_float ll_pt1, baby_float ll_pt2, 
...
vCands bmaker_boost::writePhotons(edm::Handle<pat::PhotonCollection> allphotons, 
...
bool bmaker_boost::writeTriggers(const edm::TriggerNames &names, 
                                 edm::Handle<edm::TriggerResults> triggerBits, 
                                 edm::Handle<pat::PackedTriggerPrescales> triggerPrescales){
  bool keep(false);
...
void bmaker_boost::writeHLTObjects(const edm::TriggerNames &names, 
                                   edm::Handle<pat::TriggerObjectStandAloneCollection> triggerObjects, 
...
void bmaker_boost::writeFilters(const edm::TriggerNames &fnames,
                                  edm::Handle<edm::TriggerResults> filterBits,
                                  edm::Handle<reco::VertexCollection> vtx){
...
void bmaker_boost::writeVertices(edm::Handle<reco::VertexCollection> vtx,
                                 edm::Handle<std::vector< PileupSummaryInfo > >  pu_info){  
  baby.npv() = vtx->size();
  if(pu_info.isValid()){
...
void bmaker_boost::writeGenInfo(edm::Handle<LHEEventProduct> lhe_info){
...
void bmaker_boost::writeMC(edm::Handle<reco::GenParticleCollection> genParticles, 
			   edm::Handle<edm::View<pat::PackedGenParticle> > packedParticles,
                           vCands &all_mus, vCands &all_els, vCands &photons){
  LVector isr_p4;
  ...
  //cout<<endl<<endl<<"============ Event "<<nevents<<" =============="<<endl;
  for (size_t imc(0); imc < packedParticles->size(); imc++) {
    const pat::PackedGenParticle &mc = (*packedParticles)[imc];
    size_t id = abs(mc.pdgId());
    if(id==12 || id==14 || id==16 || (id>=bsmid&&mc.charge()==0)) continue;
    baby.mcf_id().push_back(mc.pdgId());
    baby.mcf_pt().push_back(mc.pt());
    baby.mcf_eta().push_back(mc.eta());
    baby.mcf_phi().push_back(mc.phi());
    baby.mcf_mass().push_back(mc.mass());
  } // Loop over packedParticles
  ...
  for (size_t imc(0); imc < genParticles->size(); imc++) {
    ...
    if(isLast || mg_me){
	 ...
	 || tauFromTopZ || nuFromZ || fromWOrWTau || fromTop || mg_me || mc.pt()>30){
...
void bmaker_boost::rebalancedMET( double& minMET, double& minMETPhi)
{
...
double bmaker_boost::calculateRescalingFactor(unsigned int jetIdx)
{
...
double bmaker_boost::calculateRebalancedMET(unsigned int jetIdx, double mu, double& METPhi)
{
  TVector3 jet, metVector;
  jet.SetPtEtaPhi(baby.jets_pt().at(jetIdx), baby.jets_eta().at(jetIdx), baby.jets_phi().at(jetIdx));
  metVector.SetPtEtaPhi(baby.met(), 0, baby.met_phi());
...
void bmaker_boost::writeWeights(const vCands &sig_leps, edm::Handle<GenEventInfoProduct> &gen_event_info, 
                                edm::Handle<LHEEventProduct> &lhe_info){
...
bmaker_boost::bmaker_boost(const edm::ParameterSet& iConfig):
...
void bmaker_boost::reportTime(const edm::Event& iEvent){
  // Time reporting
  if(nevents==1) {
    time_t curTime;
    time(&curTime);
...
void bmaker_boost::beginJob() {
}

// ------------ method called once each job just after ending the event loop  ------------
void bmaker_boost::endJob() {
}

// ------------ method called when starting to processes a run  ------------
/*
void 
bmaker_boost::beginRun(edm::Run const&, edm::EventSetup const&)
{
}
*/
...
/*
void 
bmaker_boost::endRun(edm::Run const&, edm::EventSetup const&)
{
}
*/
...
/*
void 
bmaker_boost::beginLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}
*/
...
/*
void 
bmaker_boost::endLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}
*/
...
void
bmaker_boost::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(bmaker_boost);

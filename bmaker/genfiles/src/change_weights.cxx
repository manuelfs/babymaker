...
int main(int argc, char *argv[]){
  ...
  for(unsigned int idx=0;idx<sum_bctag.size();idx++)        var_val[4].push_back("*"+to_string(nent/sum_bctag[idx]));
  for(unsigned int idx=0;idx<sum_udsgtag.size();idx++)      var_val[5].push_back("*"+to_string(nent/sum_udsgtag[idx]));
  for(unsigned int idx=0;idx<sum_fs_bctag.size();idx++)     var_val[6].push_back("*"+to_string(nent/sum_fs_bctag[idx]));
  for(unsigned int idx=0;idx<sum_fs_udsgtag.size();idx++)   var_val[7].push_back("*"+to_string(nent/sum_fs_udsgtag[idx]));
  for(unsigned int idx=0;idx<sum_isr.size();idx++)          var_val[8].push_back("*"+to_string(nent/sum_isr[idx]));
  for(unsigned int idx=0;idx<sum_spdf.size();idx++)         var_val[9].push_back("*"+to_string(nent/sum_spdf[idx]));
  for(unsigned int idx=0;idx<sum_mur.size();idx++)          var_val[10].push_back("*"+to_string(nent/sum_mur[idx]));
  for(unsigned int idx=0;idx<sum_muf.size();idx++)          var_val[11].push_back("*"+to_string(nent/sum_muf[idx]));
  for(unsigned int idx=0;idx<sum_murf.size();idx++)         var_val[12].push_back("*"+to_string(nent/sum_murf[idx]));
  //Calculate lepton weights
  var_val[13].push_back("*"+to_string((nent-sum_wlep)/nent_zlep));
  var_val[14].push_back("*"+to_string((nent-sum_fs_wlep)/nent_zlep));
  for(unsigned int idx=0;idx<sum_slep.size();idx++)         
    var_val[15].push_back("*"+to_string((nent-sum_slep[idx])/nent_zlep));
  for(unsigned int idx=0;idx<sum_fs_slep.size();idx++)      
    var_val[16].push_back("*"+to_string((nent-sum_fs_slep[idx])/nent_zlep));
  var_val[17].push_back("*"+to_string(w_lumi_corr));
